...
import {
  ...
  Plus, Eye, Edit3, Sliders, Swords, Shield, Clock, TrendingUp,
  Menu, X, Zap, Loader2
} from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { id: "/teams",       label: "Teams",       icon: Shield,          path: "/teams"       },
...
export default function ETourneyDashboard() {
  ...
  // Auth user dari localStorage (tidak perlu fetch /me)
  ...
  const Sidebar = () => (
    <aside className={`fixed top-0 left-0 h-screen z-30 flex flex-col bg-[#0d0d0f] border-r border-white/5 transition-all duration-300 ease-in-out ${sidebarOpen ? "w-64" : "w-0 overflow-hidden md:w-16"}`}>
      <div className="flex items-center gap-3 px-5 py-5 border-b border-white/5 shrink-0">
        ...
        {sidebarOpen && <span className="text-white font-black tracking-wider text-lg whitespace-nowrap">E<span className="text-emerald-400">-</span>TOURNEY</span>}
      </div>
      <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
        {NAV_ITEMS.map(({ id, label, icon: Icon, path }) => {
          const isActive = activeNav === id;
          return (
            <button key={id} onClick={() => { setActiveNav(id); navigate(path); }}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-150 group relative overflow-hidden ${isActive ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "text-gray-500 hover:text-gray-200 hover:bg-white/5 border border-transparent"}`}>
              {isActive && <span className="absolute left-0 top-2 bottom-2 w-0.5 bg-emerald-400 rounded-full" />}
              <Icon size={18} className={`shrink-0 ${isActive ? "text-emerald-400" : "group-hover:text-gray-200"}`} />
          ...
          {sidebarOpen && (
            <>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-emerald-400 capitalize">{authUser.role ?? "administrator"}</p>
              </div>
              <button onClick={handleLogout} className="text-gray-600 hover:text-rose-400 transition-colors p-1"><LogOut size={15} /></button>
            </>
          )}
        </div>
      </div>
  ...
  const StatsGrid = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      {statsCards.map(({ label, value, icon: Icon, iconBg, iconColor, border, glow }) => (
        <div key={label} className={`relative bg-[#111113] border ${border} rounded-xl p-5 shadow-lg ${glow} hover:shadow-xl transition-all duration-200 hover:-translate-y-0.5`}>
                ...
                return (
                  <tr key={t.id} className="hover:bg-white/[0.025] transition-colors group">
                    ...
                    <td className="px-5 py-3.5">
                      ...
                      <p className="text-xs text-gray-600 mt-0.5">{t.game}</p>
                    </td>
                    <td className="px-5 py-3.5">
        ...
        <div className="p-5 lg:p-7 max-w-screen-2xl mx-auto space-y-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Selamat datang kembali, {authUser.name ?? "Admin"} · {new Date().toLocaleDateString("id-ID", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</p>
            </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            <div className="xl:col-span-2"><TournamentsTable /></div>
            <div className="xl:col-span-1"><RecentRegistrations /></div>
          </div>
        </div>
      </main>
    </div>
