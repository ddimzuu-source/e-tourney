...
import {
  ...
  Users, Settings, LogOut, Menu, Eye, X
} from "lucide-react";
    ...
    <aside className={`fixed top-0 left-0 h-screen z-30 flex flex-col bg-[#0d0d0f] border-r border-white/5 transition-all duration-300 ${sidebarOpen ? "w-64" : "w-16"}`}>
      ...
      <nav className="flex-1 py-4 px-2 space-y-1">
        {NAV_ITEMS.map(({ id, label, icon: Icon }) => {
          const isActive = id === "/payments";
          return (
            <button key={id} onClick={() => navigate(id)}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-150 relative ${isActive ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" : "text-gray-500 hover:text-gray-200 hover:bg-white/5 border border-transparent"}`}>
              {isActive && <span className="absolute left-0 top-2 bottom-2 w-0.5 bg-amber-400 rounded-full" />}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/5">
                      {["Tim", "Turnamen", "Jumlah", "Metode", "Status", "Tanggal", "Aksi"].map(h => (
                    ...
                    {filtered.map(p => {
                      ...
                      return (
                        <tr key={p.id} className="hover:bg-white/[0.025] transition-colors group">
                          ...
                          <td className="px-5 py-4 text-gray-400 text-xs">{p.tournament_id ?? "-"}</td>
                          ...
                          <td className="px-5 py-4 text-gray-500 text-xs">
                            {p.paid_at ? new Date(p.paid_at).toLocaleDateString("id-ID") : p.created_at ? new Date(p.created_at).toLocaleDateString("id-ID") : "-"}
                          </td>
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-1.5">
                              <button onClick={() => setViewData(p)} className="p-1.5 rounded-lg hover:bg-white/10 text-gray-500 hover:text-cyan-400 transition-colors" title="Detail"><Eye size={15} /></button>
                              {p.status === "pending" && (
                                <>
                                  <button onClick={() => handleUpdateStatus(p.id, "paid")} className="p-1.5 rounded-lg hover:bg-white/10 text-gray-500 hover:text-emerald-400 transition-colors" title="Konfirmasi"><CheckCircle size={15} /></button>
                                  <button onClick={() => handleUpdateStatus(p.id, "rejected")} className="p-1.5 rounded-lg hover:bg-white/10 text-gray-500 hover:text-rose-400 transition-colors" title="Tolak"><XCircle size={15} /></button>
                                </>
                              )}
                            </div>
                          </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-[#111113] border border-white/10 rounded-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-3">
              {[
                { label: "ID Transaksi", value: viewData.id },
                { label: "Tim", value: viewData.team_id },
                { label: "Turnamen", value: viewData.tournament_id },
                { label: "Jumlah", value: `Rp ${Number(viewData.amount || 0).toLocaleString("id-ID")}` },
                { label: "Metode Pembayaran", value: viewData.payment_method },
                { label: "Status", value: viewData.status },
                { label: "Tanggal Bayar", value: viewData.paid_at ? new Date(viewData.paid_at).toLocaleString("id-ID") : "-" },
              ].map(({ label, value }) => (
            ...
            {viewData.status === "pending" && (
              <div className="flex gap-3 px-6 py-4 border-t border-white/5">
                <button onClick={() => handleUpdateStatus(viewData.id, "rejected")}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-rose-500/20 border border-rose-500/30 text-rose-400 text-sm font-semibold">
                  <XCircle size={15} /> Tolak
                </button>
                <button onClick={() => handleUpdateStatus(viewData.id, "paid")}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-sm font-semibold">
